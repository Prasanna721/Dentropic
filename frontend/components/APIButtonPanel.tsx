...
// Default params for each endpoint
const ENDPOINT_DEFAULT_PARAMS: Record<string, Record<string, string>> = {
  reports: { patient_name: "Jane Smith" },
  patient_chart: { patient_name: "Jane Smith" },
};
      ...
      <div className="flex flex-wrap gap-2">
        {API_ENDPOINTS.map((endpoint) => {
          ...
          return (
            <button
              key={endpoint.id}
              onClick={() => onRunAPI(endpoint.id, ENDPOINT_DEFAULT_PARAMS[endpoint.id])}
              ...
              className={`
                ...
                ${
                  !isConnected || isRunning
                    ...
                    : "bg-[#333333] text-[#16A34A] hover:bg-[#444444] cursor-pointer"
                }
              `}
            ...
            >
              {isThisRunning ? (
                <span className="flex items-center gap-2">
                  <span className="w-2 h-2 bg-[#16A34A] animate-pulse"></span>
                  Running...
                </span>
              ) : (
                endpoint.name
